...
impl DfaBuilder {
    ...
    fn swap_state(&mut self, first: u16, second: u16) {
        ...
        self.building_layers.iter_mut().flatten().map(|state| {
            *state = match *state {
...
impl StateMachineBuilder for DfaBuilder {
    ...
    fn add_state(&mut self) -> u16 {
        ...
        return self.states + (self.building_layers.len() / self.chars as usize) as u16;
    }
...
impl Dfa {
    pub fn build(
        ...
        if transition_table.len() != ((states + 1) * (chars + 1)) as usize {
            return Err(());
        }
        if transition_table
            .iter()
